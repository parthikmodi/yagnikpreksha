...
jQuery(document).ready(function () {
    ...
    theme_conrols.on('click', '.customizer-repeater-new-field', function () {
        ...
        {
        if(jQuery('#exist_eventpress_Slider').val()>=1)
        {
        ...
        return false;   
         ...
         if(jQuery('#exist_eventpress_Slider').val()<1)
        {
         var new_service_add_val=parseInt(jQuery('#exist_eventpress_Slider').val())+1;
         jQuery('#exist_eventpress_Slider').val(new_service_add_val);  
        }
        }
		
		if(split_add_more_button_split=="Add New Orga")
        {
        if(jQuery('#exist_eventpress_Organizer').val()>=2)
        {
        jQuery(".customizer_org_upgrade_section").show();
        return false;   
        }
         if(jQuery('#exist_eventpress_Organizer').val()<2)
        {
         var new_service_add_val=parseInt(jQuery('#exist_eventpress_Organizer').val())+1;
         jQuery('#exist_eventpress_Organizer').val(new_service_add_val);  
        }
        }
    ...
    theme_conrols.on('click', '.social-repeater-general-control-remove-field', function () {
        ...
        if (typeof    jQuery(this).parent() !== 'undefined') {
            jQuery(this).parent().hide(500, function(){
                ...
                {
                jQuery('#exist_eventpress_Slider').val(parseInt(jQuery('#exist_eventpress_Slider').val())-1);  
                }
				
				if(split_delete_button_split=="Delete Organ")
                {
                jQuery('#exist_eventpress_Organizer').val(parseInt(jQuery('#exist_eventpress_Organizer').val())-1);  
                }
